module SolutionsHelper
  def new_solution_button(idea)
    if idea.status != IDEA_STATUS_LAUNCHED
      cssClass = user_signed_in? ? "solution-btn":"login-required"
      content_tag :div,:class=>"btn-group" do
        button_tag I18n.t("app.solution.new"),:class=>cssClass+" btn btn-primary","data-idea"=> idea.id
      end
    end
  end
